...
def deletenotes(nid):
    if session.get('user'):
        ...
        return redirect(url_for('login'))
@app.route('/uplaodfile',methods=['GET','POST'])    
def uploadfile():
    if session.get('user'):
        try:
            if request.method=='POST':
...
def viewfiles(fid):
    if session.get('user'):
        try:
            ...
            byte_data=BytesIO(filedata[1])
            return send_file(byte_data,download_name=filedata[0],as_attachment=False)
        except Exception as e:
            print(e)
